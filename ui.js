/* ui.js — FINAL GOLD: OPACITY & INVENTORY */
(function () {
  ...
  const TUTORIAL_KEY = "nx_tutorial_final";
    ...
    A.forEach((h) => {
      ...
      if (pct >= 1) {
          ...
          if (slot && !document.getElementById("tap-" + h.id)) {
              const tap = document.createElement("div"); tap.id = "tap-" + h.id; tap.innerText = "TAP!";
  ...
  function updateStats(){if(!IS_GAME)return;const b=el("disciple-hp-bar"),l=el("disciple-hp-label"),m=el("moves-left");if(b&&GS.discipleHP!=null){const p=Math.max(0,GS.discipleHP/GS.discipleMaxHP);b.style.width=(p*100)+"%"}if(l&&GS.discipleHP!=null)l.textContent=GS.discipleHP+" HP";if(m)m.textContent=GS.movesLeft;if(GS.currentLevelId){const l=GS.currentLevelId,z=Math.ceil(l/10);const e=document.getElementById("level-indicator");if(e)e.innerText=`SEC ${z}-${l}`}if(GS.timeLeft){const t=document.getElementById("time-indicator");if(t)t.innerText=`TIME: ${GS.timeLeft}s`}}
  function updateEnergyUI(){if(!window.economy)return;window.economy.regenerateEnergy();const c=window.economy.getEnergy(),m=window.economy.maxEnergy,e=el("energy-count");if(e)e.textContent=c;}
