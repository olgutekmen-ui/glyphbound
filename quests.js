/* quests.js — V2.3 (ACTION BUTTON UX) */
(function() {
    ...
    const Quests = {
        ...
        renderModal() {
            ...
            let actionBtnText = isMap ? "START MISSION" : "RESUME MISSION";
            let actionBtnAction = "document.getElementById('quest-modal').style.display='none'"; // Default close

            // Logic for the Main Button (Claim vs Action)
            let mainBtn = "";
            if (isClaimed) {
                // Done. Show Action Button.
                mainBtn = `<button onclick="${actionBtnAction}" style="background:#0ea5e9; color:#fff; border:none; padding:12px 40px; border-radius:8px; font-weight:bold; letter-spacing:1px; cursor:pointer; width:100%; margin-top:10px;">${actionBtnText}</button>`;
            } else if (isDone) {
                // Ready to Claim
                mainBtn = `<button onclick="Quests.claim()" style="background:linear-gradient(135deg, #22c55e, #16a34a); color:#fff; border:none; padding:12px 40px; border-radius:8px; font-weight:900; letter-spacing:1px; cursor:pointer; box-shadow:0 0 20px rgba(34, 197, 94, 0.6); transform:scale(1.05); width:100%;">CLAIM REWARD</button>`;
            } else {
                // In Progress. Show Action Button to let them play.
                mainBtn = `<button onclick="${actionBtnAction}" style="background:transparent; border:1px solid #38bdf8; color:#38bdf8; padding:12px 40px; border-radius:8px; font-weight:bold; letter-spacing:1px; cursor:pointer; width:100%; margin-top:10px;">${actionBtnText}</button>`;
            }
            ...
            WEEKLY_CONFIG.forEach(wk => {
                ...
                if (active) { imgStyle += " filter: none; transform: scale(1.1); box-shadow: 0 0 10px #facc15;"; borderStyle = "2px solid #facc15"; } 
                else if (past) { imgStyle += " filter: grayscale(0.5) brightness(0.6);"; borderStyle = "2px solid #38bdf8"; } 
                else { imgStyle += " filter: grayscale(1) brightness(0.3);"; borderStyle = "2px solid #334155"; }
                stripHTML += `<div style="border:${borderStyle}; border-radius:8px; overflow:hidden; padding:2px; background:#020617;"><img src="assets/quest${wk.day}.png" style="${imgStyle}" onerror="this.style.backgroundColor='#333'"></div>`;
            });
            stripHTML += `</div>`;
            ...
            el.innerHTML = `
                ...
                ">
                    ...
                    <div style="margin-bottom:20px; position:relative;">
                        <div style="font-size:0.8rem; color:#94a3b8; letter-spacing:2px; text-transform:uppercase; margin-bottom:10px;">Day ${q.day} Protocol</div>
