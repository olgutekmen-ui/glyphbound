...
(function () {
  ...
  const drag = { active: false, startR: -1, startC: -1, startX: 0, startY: 0, pointerId: null, swapped: false };
  let selected = null;
  ...
  function onCellPointerDown(e, r, c) {
    if (window.AudioSys && AudioSys.ctx && AudioSys.ctx.state === 'suspended') { AudioSys.ctx.resume().then(() => { if (!AudioSys.muted) AudioSys.playBGM('bgm_battle'); }); }
    if (GS.isProcessing || !window.Board) return;
    const cell = GS.board?.[r]?.[c];
    if (!isSwappable(cell)) { clearSelection(); return; }
    drag.active = true; drag.pointerId = e.pointerId; drag.startR = r; drag.startC = c; drag.startX = e.clientX; drag.startY = e.clientY; drag.swapped = false;
  ...
  function onMove(e) {
    ...
    drag.swapped = true; clearSelection();
  ...
  function onUp(e) {
    ...
    if (!drag.swapped && !GS.isProcessing) {
      ...
      if (!selected) { setSelection(r, c); } else {
        if (selected.r === r && selected.c === c) { clearSelection(); } 
        else if (areNeighbors(selected.r, selected.c, r, c)) { const r1 = selected.r, c1 = selected.c; clearSelection(); try { if(window.AudioSys && AudioSys.play) AudioSys.play('swap'); } catch(e){} window.Engine.trySwap(r1, c1, r, c); } 
        else { setSelection(r, c); }
      }
    }
    resetDrag(); if(e.cancelable) e.preventDefault();
  }
  function onCancel(e) { window.removeEventListener("pointermove", onMove); window.removeEventListener("pointerup", onUp); window.removeEventListener("pointercancel", onCancel); resetDrag(); clearSelection(); }
