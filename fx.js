/* fx.js — V1.4 (COMBO VISUALS ADDED) */
(function() {
    const FX = {
        ...
        // --- NEW: COMBO VISUAL ---
        showCombo(count) {
            // Remove existing combo text to prevent clutter
            const existing = document.getElementById("active-combo-text");
            if(existing) existing.remove();

            // Only show if it's actually a combo (2+)
            if (count < 2) return;
        ...
        explode(r, c, typeIndex) {
            ...
            for (let i = 0; i < 6; i++) this.createParticle(centerX, centerY, color);
        },
